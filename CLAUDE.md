...
- **Chart scrubbing has to lose the gesture gracefully.** The chart sits inside the home
  `LazyColumn`, so its `pointerInput` shows the readout on touch-down but only *consumes* pointer
  changes once the drag reads horizontal (past touch slop, |dx| > |dy|). A vertical swipe that
  starts on the chart must still scroll the page, so it bails out of the scrub instead.
